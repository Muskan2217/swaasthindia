...
import {
  User, Phone, Mail, Stethoscope, Briefcase, Building2,
  Hash, ArrowRight, AlertCircle,
} from "lucide-react";
import {
...
import { InputField, PasswordField, SelectField, FileUploadField } from "./FormField";
...
function validate(data: DoctorSignupFormData): Errors {
  ...
  if (!/^\d{10}$/.test(data.mobile)) errors.mobile = "Enter a valid 10-digit mobile number.";
  ...
  if (!data.specialization) errors.specialization = "Please select a specialization.";
  ...
  if (!data.registrationNumber.trim()) errors.registrationNumber = "Medical registration number is required.";
  if (!data.profilePhoto) errors.profilePhoto = "Profile photo is required.";
  if (!data.degreeCertificate) errors.degreeCertificate = "Degree certificate is required.";
  if (!data.acceptTerms) errors.acceptTerms = "You must accept the Terms & Conditions.";
  return errors;
}

export default function DoctorSignupForm() {
  const router = useRouter();
  const [form, setForm] = useState<DoctorSignupFormData>(DOCTOR_SIGNUP_DEFAULTS);
  const [errors, setErrors] = useState<Errors>({});
  const [loading, setLoading] = useState(false);

  const set = (key: keyof DoctorSignupFormData) => (v: string | boolean | File | null) =>
    setForm((f) => ({ ...f, [key]: v }));
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-4">
      <div className="mb-1">
        <h1 className="text-2xl font-extrabold text-[#0D1B3E]">Doctor Registration</h1>
        <p className="text-sm text-gray-500 mt-1">
          Submit your credentials. Verification typically takes 1–2 working days.
        </p>
      </div>
      ...
      <InputField
        ...
        placeholder="Swasth Heart & Care Clinic"
      ...
      <div className="flex flex-col gap-1">
        <label className="flex items-start gap-2.5 cursor-pointer select-none">
          ...
          <span className="text-sm text-gray-600 font-medium leading-relaxed">
            I accept the{" "}
            <Link href="/terms" className="text-[#3864D5] font-semibold hover:underline">
              Terms & Conditions
            </Link>{" "}
        ...
        {errors.acceptTerms && (
          <p className="text-xs text-red-500 font-medium pl-6">{errors.acceptTerms}</p>
        )}
      </div>
      ...
      <p className="text-center text-sm text-gray-500">
        Already registered?{" "}
        <Link href="/login" className="font-bold text-[#3864D5] hover:text-[#2450B0] transition-colors">
          Sign in
        </Link>
      </p>
    </form>
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex items-center gap-3 pt-1">
      <span className="text-xs font-bold text-[#3864D5] uppercase tracking-widest">{children}</span>
