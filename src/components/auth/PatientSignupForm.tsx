...
import { User, Phone, Mail, CheckCircle2, ArrowRight, AlertCircle } from "lucide-react";
import {
...
export default function PatientSignupForm() {
  ...
  const [form, setForm] = useState<PatientSignupFormData>(PATIENT_SIGNUP_DEFAULTS);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Simulate API: await fetch('/api/auth/register/patient', { method: 'POST', body: JSON.stringify(form) })
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-4">
      <div className="mb-1">
        <h1 className="text-2xl font-extrabold text-[#0D1B3E]">Patient Registration</h1>
      ...
      <div className="flex flex-col gap-1">
        <label className="flex items-start gap-2.5 cursor-pointer select-none">
          ...
          <span className="text-sm text-gray-600 font-medium leading-relaxed">
            ...
            and{" "}
            <Link href="/privacy" className="text-[#3864D5] font-semibold hover:underline">
              Privacy Policy
            </Link>
          </span>
        </label>
